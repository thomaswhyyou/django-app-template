...
- app: Docker container based on the official python image plus Django installed and scaffolded app.
- docker-compose.yml: runs the configured Docker containers with uWSGI and nginx (default by docker-compose)
- docker-compose-dev.yml: runs the configured Docker containers with Django development server via manage.py command
